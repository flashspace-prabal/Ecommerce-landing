...
export const OnDemandDashboardPreview = () => {
  return (
    <section id="od-hero-dashboard" className="relative w-full bg-[#f5f5f5]">
          ...
          <div className="rounded-2xl border border-foreground/10 bg-background shadow-2xl overflow-hidden p-1">
            <div className="rounded-xl bg-background/90 backdrop-blur-sm p-6 sm:p-8">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                {["Desk Bookings", "Upcoming Meetings", "Events Tomorrow", "Spaces Available"].map((label, i) => (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                {["Instant Booking", "Desk Availability", "Meeting Room Access", "Member Pass"].map((pill, i) => (
                  <div key={i} className="rounded-lg bg-primary/8 border border-primary/20 px-3 py-2 text-center">
                    <p className="text-[11px] font-medium text-primary">{pill}</p>
                  </div>
                ))}
              </div>
